...
def parse_entry_name(entry_name: str) -> SpecimenEntry | None:
    ...
    # Resin mapping (used in the manuscript: sem resina / camada simples / camada dupla)
...
def plot_resin_taboa_from_zip(zip_path: Path, output_path: Path) -> None:
    with zipfile.ZipFile(zip_path, 'r') as z:
        entries: list[SpecimenEntry] = []
        for name in z.namelist():
            parsed = parse_entry_name(name)
            if parsed and parsed.species == 'Typha domingensis':
                entries.append(parsed)

        if not entries:
            raise RuntimeError('No Taboa resin specimen curves found in zip')
        ...
        fig.suptitle('Punção (CBR) — Taboa (Typha domingensis) — Resina', y=1.05, fontsize=13, fontweight='bold')
...
def main() -> None:
    ...
    out_path = base / r"3-IMAGENS\puncao_taboa_resina_curvas.png"

    plot_resin_taboa_from_zip(zip_path, out_path)
