...
The lignin-cellulose ratio (L/C) establishes itself as the principal predictive index of recalcitrance [@Santos2023_GeocompostosTypha]. Empirical observations in our dataset demonstrate an inverse exponential relationship between L/C and the degradation rate constant ($k$, day⁻¹), given by [@Ref4; @Ref5]:
...
The ubiquity of hydroxyl functional groups (-OH) on cellulose and hemicellulose surfaces tends to confer a hydrophilic character to fibers, often associated with low water contact angles, a condition that facilitates moisture entry and creates microenvironments favorable to microbial colonization in the early stages of field exposure [@Chieng2017].
...
While mercerization optimizes internal structure, external protection relies on barrier strategies. To quantify the temporal trend observed in Figure 6, a simple linear regression was fitted between exposure time (days) and mean UTS (N/mm²), by species and by treatment condition (means by exposure time).

In *Typha domingensis*, the resin monolayer exhibited a steep linear decline between 30–120 days: for each additional day of exposure, there was a mean reduction of 0.084 N/mm² in UTS (β = −0.084; t = −10.339; p = 0.009; R² = 0.982; 98.2% of variance explained). The control (untreated) showed a weak, non-significant trend (β = −0.034; t = −1.128; p = 0.377; R² = 0.389; 38.9% of variance explained), while the bilayer showed a moderate decline, but without robust statistical evidence in the available interval (β = −0.046; t = −2.767; p = 0.110; R² = 0.793; 79% of variance explained).

In *Syagrus coronata*, the series presented negative coefficients (β = −0.221 N/mm²·day⁻¹), but without statistical significance when fitted on means by time (p ≥ 0.126), with moderate R² values (R² = 0.596–0.695; 59–69% of variance explained). This pattern is consistent with a less linear trajectory (rapid initial drop and stabilization at low levels), in addition to gaps at later times, which reduces inferential power when the fit is performed on time points.
...
Figure 6b showed that exposure time had a significant negative effect on UTS in *Typha domingensis* at all NaOH concentrations (β ranging from −0.021 to −0.057 N/mm²·day⁻¹; p ≤ 0.010; R² = 0.844–0.922; 84.4–92.2% of variance explained), indicating consistent temporal degradation. In *Syagrus coronata*, the effect was significant only at 6% and 9% NaOH (β = −0.099; t = −3.955; p = 0.029; R² = 0.83; 83.9% of variance explained; and β = −0.108; t = −5.067; p = 0.015; R² = 0.895; 89.5% of variance explained), whereas 0% and 3% did not show statistically robust association (p ≥ 0.144).
